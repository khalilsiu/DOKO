...
const useRenderMaps = ({ bounds, items, selected, center }: IProps) => {
  ...
  useEffect(() => {
    if (!map) return;
    map.closePopup();
    if (markerRefs && selected !== null && markerRefs.current[selected] && markerRefs.current[selected].ref) {
  ...
  useEffect(() => {
    if (map && bounds) {
      const newBounds = new L.LatLngBounds(map.unproject(bounds.southwest, 0), map.unproject(bounds.northeast, 0));
