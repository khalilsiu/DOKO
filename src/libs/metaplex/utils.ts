...
const solanaRpcUrl =
  'https://black-fragrant-tree.solana-mainnet.quiknode.pro/580c20b14f5a8fb782624bdce35ee63037ee3d3c/';
export const solanaConnection = new Connection(solanaRpcUrl);
const TOKEN_PROGRAM_ID = 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA';
const METADATA_PROGRAM_ID = 'metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s';

const isMetadataAccount = (account: AccountInfo<Buffer>) => account.owner.toBase58() === METADATA_PROGRAM_ID;

const isMetadataV1Account = (account: AccountInfo<Buffer>) => account.data[0] === MetadataKey.MetadataV1;
...
export const getTokenInfo = async (mintAddress: string) => {
  try {
    ...
    const tokenProgramAddressRes = await PublicKey.findProgramAddress(seed, new PublicKey(METADATA_PROGRAM_ID));
