...
export default function MetaCollection() {
  ...
  const fetchNfts = async () => {
    ...
    try {
      const {
        data: { assets },
      } = await (isSolAddress(address) ? getSolNfts(address, (page - 1) * 12) : getEthNFTs(address, (page - 1) * 12));
  ...
  return (
    <div>
      ...
      <div className={styles.collectionContainer}>
        {tab === 0 && (
          <CollectionTab setPage={setPage} nfts={nfts} loading={loading} page={page} collection={collection} />
        )}
