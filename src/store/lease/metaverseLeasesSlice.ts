...
export const upsertLeaseToBlockchain = createAsyncThunk<void, IUpsertLease, { rejectValue: ThunkError }>(
  ...
  async ({ leaseForm, walletAddress, assetId, dclLandRentalContract, isUpdate }: IUpsertLease, { rejectWithValue }) => {
    try {
      ...
      const lease: LeasePayload = [walletAddress, '0x0000000000000000000000000000000000000000', assetId];
...
export const acceptLeaseToBlockchain = createAsyncThunk<void, IAcceptLease, { rejectValue: ThunkError }>(
  'MetaverseLeases/acceptLeaseToBlockchain',
  async ({ assetId, finalLeaseLength, dclLandRentalContract }: IAcceptLease, { rejectWithValue, getState }) => {
    try {
...
export const getMetaverseLeases = createAsyncThunk('MetaverseLeases/getLesaes', async (payload: any) => {
